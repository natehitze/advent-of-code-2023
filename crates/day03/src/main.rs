fn main() {
    let input = ".....180.........230..........................218.....189......415.......................322....507..................206..............111...
...
";
    ...
    for line in input.lines() {
        let row = parse_row_line(line, row_index);
        rows.push(row);
        row_index += 1;
    }

    // Walk the rows to mark parts
    let row_count = rows.len();
    for i in 0..row_count {
        {
            let row = rows.get_mut(i).unwrap();
            for symbol in &mut row.symbols {
                mark_numbers_in_row(&mut row.numbers, symbol);
            }
        }

        if i > 0 {
            let (previous_slice, current_slice) = rows.split_at_mut(i);
            let previous_row = previous_slice.last_mut().unwrap();
            for symbol in &mut current_slice[0].symbols {
                mark_numbers_in_row(&mut previous_row.numbers, symbol);
            }
        }

        if i < row_count - 1 {
            let (current_slice, next_slice) = rows.split_at_mut(i+1);
            let next_row = &mut next_slice[0];
            for symbol in &mut current_slice.last_mut().unwrap().symbols {
                mark_numbers_in_row(&mut next_row.numbers, symbol);
            }
        }
    }

    // Walk the rows again to sum the parts
    let mut part_1_sum = 0;
    for row in &rows {
        for number in &row.numbers {
            if number.is_part_number {
                part_1_sum += number.value;
            }
        }
    }

    // Walk the rows again to find gears
    let mut part_2_sum = 0;
    for row in &rows {
        for symbol in &row.symbols {
            if symbol.char == '*' && symbol.adjacent_parts.len() == 2 {
                part_2_sum += symbol.adjacent_parts[0] * symbol.adjacent_parts[1];
            }
        }
    }

    println!("Part 1 answer: {part_1_sum}");
    println!("Part 2 answer: {part_2_sum}");
}
...
struct Symbol {
    char: char,
    index: i32,
    adjacent_parts: Vec<i32>,
}

fn i_between(value: i32, inclusive_start: i32, inclusive_end: i32) -> bool {
    value >= inclusive_start && value <= inclusive_end
}

fn mark_numbers_in_row(row_numbers: &mut Vec<Number>, symbol: &mut Symbol) {
    let start_index = symbol.index - 1;
    let end_index = symbol.index + 1;
    for number in row_numbers {
        if i_between(number.start_index, start_index, end_index) || i_between(number.end_index, start_index, end_index) {
            number.is_part_number = true;
            symbol.adjacent_parts.push(number.value);
        }
    }
}

fn convert_to_number(numerals: &mut Vec<i32>, current_index: i32) -> Number {
    let mut number_value = 0i32;

    if numerals.len() == 0 {
        panic!("Empty numerals");
    }

    let mut i = 0;
    numerals.reverse();
    for n in &mut *numerals {
        number_value += *n * (10_i32.pow(i));
        i += 1;
    }
    ...
    let result = Number {
        ...
        start_index: current_index - numerals.len() as i32,
...
fn parse_row_line(line: &str, row_index: i32) -> Row {
    let mut numbers: Vec<Number> = Vec::new();
    let mut symbols: Vec<Symbol> = Vec::new();

    let mut index = 0i32;
    let mut numerals: Vec<i32> = Vec::new();
    for c in line.chars() {
        if let Some(d) = c.to_digit(10) {
            numerals.push(d as i32);
        }
        else if c.is_alphabetic() {
            panic!("Letters aren't expected");
        }
        else {
            if c != '.' {
                // symbol
            ...
            if numerals.len() > 0 {
                numbers.push(convert_to_number(&mut numerals, index));
            }
        }
...
#[cfg(test)]
mod tests {
    use crate::{i_between, parse_row_line};

    #[test]
    fn parse_row_line_blank() {
        let row= parse_row_line("......", 1);
        assert_eq!(row.numbers.len(), 0);
        assert_eq!(row.symbols.len(), 0);
        assert_eq!(row.row_index, 1);
    }

    #[test]
    fn parse_row_line_number_in_middle() {
        let row= parse_row_line(".114.", 1);
        assert_eq!(row.numbers.len(), 1);
        assert_eq!(row.symbols.len(), 0);
        assert_eq!(row.row_index, 1);

        assert_eq!(row.numbers[0].value, 114);
        assert_eq!(row.numbers[0].is_part_number, false);
        assert_eq!(row.numbers[0].start_index, 1);
        assert_eq!(row.numbers[0].end_index, 3);
    }

    #[test]
    fn parse_row_line_number_in_start() {
        let row= parse_row_line("987..", 1);
        assert_eq!(row.numbers.len(), 1);
        assert_eq!(row.symbols.len(), 0);
        assert_eq!(row.row_index, 1);

        assert_eq!(row.numbers[0].value, 987);
        assert_eq!(row.numbers[0].is_part_number, false);
        assert_eq!(row.numbers[0].start_index, 0);
        assert_eq!(row.numbers[0].end_index, 2);
    }

    #[test]
    fn parse_row_line_number_in_end() {
        let row= parse_row_line("..592", 1);
        assert_eq!(row.numbers.len(), 1);
        assert_eq!(row.symbols.len(), 0);
        assert_eq!(row.row_index, 1);

        assert_eq!(row.numbers[0].value, 592);
        assert_eq!(row.numbers[0].is_part_number, false);
        assert_eq!(row.numbers[0].start_index, 2);
        assert_eq!(row.numbers[0].end_index, 4);
    }

    #[test]
    fn parse_row_line_multiple_numbers() {
        let row= parse_row_line("..592..$.315", 1);
        assert_eq!(row.numbers.len(), 2);
        ...
        assert_eq!(row.numbers[1].value, 315);
        assert_eq!(row.numbers[1].is_part_number, false);
        assert_eq!(row.numbers[1].start_index, 9);
        assert_eq!(row.numbers[1].end_index, 11);
    }

    #[test]
    fn parse_row_line_symbol_in_end() {
        let row= parse_row_line("..592#", 1);
        assert_eq!(row.numbers.len(), 1);
        assert_eq!(row.symbols.len(), 1);
        assert_eq!(row.row_index, 1);
        ...
        assert_eq!(row.symbols[0].char, '#');
        assert_eq!(row.symbols[0].index, 5);
    }

    #[test]
    fn i_between_ranges() {
        assert_eq!(i_between(1, 0, 1), true);
        assert_eq!(i_between(0, 0, 1), true);
        assert_eq!(i_between(2, 0, 1), false);
        assert_eq!(i_between(2, 3, 5), false);
        assert_eq!(i_between(6, 3, 5), false);
        assert_eq!(i_between(4, 3, 5), true);
    }
}